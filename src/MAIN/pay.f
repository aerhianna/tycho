...
c         nsqr(1) = 0.0d0
         do k = 2, kk
c            bv2(k) = ( dnrad(k) - dnad(k)-0.0d0*doux(k))*g(k)**2/(
c     4              0.5d0*(p(nc,k)*v(nc,k)+p(nc,k+1)*v(nc,k+1)) )
c     1           *(v(nc,k)-v(nc,k+1))*(t(nc,k)+t(nc,k+1))*0.5d0/
c     1           ((t(nc,k)-t(nc,k+1))*(v(nc,k)+v(nc,k+1))*0.5d0)
            bv2(k) = ( -dnrad(k)+ dnad(k) +doux(k) )*
     1           (grav*xm(k)/r(nc,k)**2)**2/
...
c             if(abs((nsqr(k)-bv2(k))/bv2(k)) .lt. 0.1d0)then
c               bv2(k) = nsqr(k)
c             else
c               bv2(k) = bv2(k)
c             endif
...
c         if( bv2(kk) .gt. 0.0d0 )then
c            v2down(kk+1) = -bv2(kk)/8.0d0 *hml(kk)**2
c         else
         if( bv2(kk) .le. 0.0d0 )then
            v2down(kk+1) = 0
         endif
...
c if statement to keep convective velocities from being recalculated 
c for short timesteps
        
         do j = 1, kk-1
            k = kk +1 -j
c            if(ld(k+1)/h(k+1) .lt. dth(1))then
c            v2down(k) = v2down(k+1) -bv2(k)*dellr(k)*hml(k)
c            if(bv2(k) .gt. 0.0d0)then
c            v2down(k) = ( v2down(k+1) -bv2(k)*dellr(k)*hml(k)*1.0d0 )
c     1           *hml(k)/( hml(k) + 16.0d0*dellr(k) )
            if(ld(k+1) .ne. 0.0d0)then
            v2down(k) = ( v2down(k+1) -bv2(k)*dellr(k)**2.0d0)
     1           -v2down(k+1)**2.0d0/(0.77d0*ld(k+1))
            else
                v2down(k) = ( v2down(k+1) -bv2(k)*dellr(k)**2.0d0)
            endif
c            write(*,*)k,v2down(k),ld(k+1),dellr(k),hml(k)
c     1           *hml(k)/( hml(k) + 16.0d0*dellr(k) )
c            else
c               v2down(k) = ( v2down(k+1) -bv2(k)*dellr(k)*hml(k)*1.0d0)
c     1           *hml(k)/( hml(k) + 16.0d0*dellr(k) )
c            endif
c            write(*,*)k,v2down(k),bv2(k)
...
c            v2up(k) = ( v2up(k-1) -bv2(k)*dellr(k)*hml(k)*2.0d0)
c     1           *hml(k)/(hml(k) + 16.0d0*dellr(k) )
            if(ld(k-1) .ne. 0.0d0)then
            v2up(k) = ( v2up(k-1) -bv2(k)*dellr(k)*2.0d0)
     1           -v2up(k-1)**2.0d0/(0.77d0*ld(k-1))
            else
               v2up(k) = v2up(k-1) - bv2(k)*dellr(k)**2.0d0
            endif
...
c                  endif
     ...
     1                 (4.0d0*nsqr(k) - omegc**2.0d0)/nsqr(k))
                  if(nsqr(k) .le. 0.0d0)then
                     qfak(k) = 1.0d-14
                  endif
                  if((nsqr(k)/omegc**2.0d0) .le. 0.25d0)then
                     qfak(k) = 1.0d0 
                  endif
                  kv(k) = kh(k)*(nsqr(k)/omegc**2.0d0 -0.25d0)**0.5d0
c                  write(*,'(a20,1p8e12.3)')'kv 1',kv(k),
c     1                 nsqr(k),kh(k),omegc
cccccccccccccccccccc

                  if(4.0d0*nsqr(k)-omegc**2.0d0 .gt. 0.0d0)then
                     reflec = k
                  else
...
cccccccccccccccccccc
     ...
     1                 (4.0d0*nsqr(k)-omegc**2.0d0)
                  if((nsqr(k)/omegc**2.0d0 -0.25d0) .le. 0.0d0)then
