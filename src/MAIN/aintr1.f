      subroutine aintr1(qqo,qqn,f,scr,jj,njj)
      implicit none
...
c     does quadratic  interpolation; 
c     use lintr1 for functions that must be .ge. 0
c     will extrapolate below qqo(1) or above

c     note that f is changed upon return

      include 'dimenfile'

      integer*4 jj,njj,n,k,ki
...
c..   quadratic interpolation
     ...
     1        ( (qqo(k+2) - qqo(k+1))*(qqo(k+2) - qqo(k)  ) )
         scr(n) = fact1*f(k) + fact2*f(k+1) + fact3*f(k+2)
      enddo
