...
c      data fontsize/1.2/,pgscal/1.0/
c     higher resolution (more points, so closer points)
      data fontsize/1.2/,pgscal/1.20/
...
c..   font scaling (size) for first line
      ...
      if( nopac .eq. -1 )then
         call PGMTXT('T', tabhi(1), tabul(7), fjust, 'nopac')
      else
         call PGMTXT('T', tabhi(1), tabul(7), fjust, copal)
      endif
      ...
      call itoa(mode ,cmode)
      call itoa(modec,cmodec)
      call itoa(modes,cmodes)
      cmodeall = 'mode( cs)'//' '//cmode//' '//cmodec//' '//cmodes
      call PGMTXT('T', tabhi(1), tabul(10), fjust, cmodeall)
...
c..   reset y array                         (nuclear heating )
      do i=1, nx
c         yy(i) = ss(i+1) 
         yy(i) = ss(i+1) + snu(i+1)
